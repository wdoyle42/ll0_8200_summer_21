...
Our first formula will have the percent of the population with incomes over 75,000 as the dependent variable and the percent of the population with a college education as the independent variable. 
...
The scatterplot above shows the percent of the population with incomes above 75,000 on the y axis and the percent of the population with a bachelor's degree on the x axis. I always like to be able to identify the individual units in datasets like this. To do that I use the `ggplotly` command below. 
...
To start training our model, we need to specify a formula. The dependent variable always goes on the left hand side, while the independent variable or variables always go on the right hand side. The formula below is for a model with the percent of the population with incomes above 75,000 on the left hand side, and the percent of the population with a college edducation on the right hand side. 
...
What this tells us is that for each additional percent of the population with a bachelor's degree, the percent of the population with incomes above 75,000 is predicted to increase by `r prettyNum()``
...
The rmse of `r  prettyNum(rmse_2,digits=2)` gives us a sense of how wrong the model tends to be when using just this one predictor. 

_Quick Exercise_ Run a regression using a different predictor. Calculate rmse and see if you can beat my score. 

## Multiple Regression. 

Okay, so we can see that this is somewhat predictive, but we can do better. Let's add in a second variable: the percent of people who own their own homes. 
